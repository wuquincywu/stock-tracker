...
const ITEMS: FaqItem[] = [
  {
    q: "均線通知（低於 MA5 / 站上 MA20…）是怎麼判斷的？",
    a: (
      <>
        看「今天」收盤價相對於均線的位置：收在均線之上 = 站上，收在均線之下 = 低於。只要條件成立就會通知，同一天不會重複通知同一檔股票同一條均線，但只要隔天條件還是成立（例如連續好幾天都站上 MA20），每個交易日都會再通知一次，不是只有剛好穿越均線的那一天才通知。
