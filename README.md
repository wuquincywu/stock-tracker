...
Triggered once per trading day at Taipei 20:15 (leaving buffer after institutional data is published). Requires `CRON_SECRET`. Triggered by [cron-job.org](https://cron-job.org) rather than Vercel Cron or GitHub Actions — Vercel Hobby only guarantees per-hour precision (±59 min) for cron invocations; GitHub Actions' `schedule` event proved unreliable here (never fired across three different cron times, then fired over 4 hours late the one time it did — see git history on `.github/workflows/check-alerts-cron.yml`, which is now `workflow_dispatch`-only for manual on-demand runs):
